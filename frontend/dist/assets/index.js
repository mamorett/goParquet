(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const s of document.querySelectorAll('link[rel="modulepreload"]'))d(s);new MutationObserver(s=>{for(const i of s)if(i.type==="childList")for(const o of i.addedNodes)o.tagName==="LINK"&&o.rel==="modulepreload"&&d(o)}).observe(document,{childList:!0,subtree:!0});function n(s){const i={};return s.integrity&&(i.integrity=s.integrity),s.referrerPolicy&&(i.referrerPolicy=s.referrerPolicy),s.crossOrigin==="use-credentials"?i.credentials="include":s.crossOrigin==="anonymous"?i.credentials="omit":i.credentials="same-origin",i}function d(s){if(s.ep)return;s.ep=!0;const i=n(s);fetch(s.href,i)}})();const t={dbPath:"",params:{existence_filter:"all",selected_subdirs:[],subdir_query:"",selected_prompts:[],sort_option:"created_desc",search_query:"",search_in:"filename_or_prompt",page:1,items_per_page:10},result:null,stats:null,allSubdirs:[],allPrompts:[],thumbnailSize:300,editingIndex:null};window.addEventListener("DOMContentLoaded",()=>{E(),_()});function E(){document.getElementById("app").innerHTML=`
  ...
  `}function _(){const a=document.getElementById("db-path-input");a.addEventListener("keypress",i=>{i.key==="Enter"&&y(a.value)}),document.getElementById("browse-btn").addEventListener("click",async()=>{const i=await window.go.main.App.OpenFilePicker();i&&(a.value=i,await y(i))}),document.getElementById("thumb-slider").addEventListener("input",i=>{t.thumbnailSize=parseInt(i.target.value,10),m()});const e=()=>{t.params.page=1,u()};document.getElementById("items-per-page").addEventListener("change",i=>{t.params.items_per_page=parseInt(i.target.value,10),e()}),document.querySelectorAll('input[name="existence"]').forEach(i=>{i.addEventListener("change",o=>{t.params.existence_filter=o.target.value,e()})}),document.getElementById("subdir-query").addEventListener("input",b(i=>{t.params.subdir_query=i.target.value,e()},300)),document.getElementById("sort-option").addEventListener("change",i=>{t.params.sort_option=i.target.value,e()}),document.getElementById("search-in").addEventListener("change",i=>{t.params.search_in=i.target.value,e()}),document.getElementById("search-query").addEventListener("input",b(i=>{t.params.search_query=i.target.value,e()},300)),document.addEventListener("click",i=>{document.querySelectorAll(".multi-select-dropdown").forEach(o=>{o.parentElement.contains(i.target)||o.classList.remove("open")})});const n=document.getElementById("resizer"),d=document.getElementById("sidebar");let s=!1;n.addEventListener("mousedown",i=>{s=!0,n.classList.add("active"),document.body.style.cursor="col-resize",i.preventDefault()}),document.addEventListener("mousemove",i=>{if(!s)return;const o=i.clientX;o>=150&&o<=800&&(d.style.width=o+"px")}),document.addEventListener("mouseup",()=>{s&&(s=!1,n.classList.remove("active"),document.body.style.cursor="default")})}function g(a,e){const n=document.getElementById("db-status");n.innerHTML=`<div class="banner ${a}">${e}</div>`}function L(a){g("info",'<div class="spinner"></div> Loading...')}async function y(a){L();const e=await window.go.main.App.LoadDatabase(a);if(e){g("error","❌ "+e);return}t.dbPath=a,g("success","✓ Valid database file"),await w(),await u()}async function w(){t.stats=await window.go.main.App.GetStats();const a=document.getElementById("stats-area");a.innerHTML=`
  ...
  `;e.forEach(r=>{const v=n.includes(r)?"checked":"";c+=`
      <label class="multi-select-option">
        <input type="checkbox" value="${r.replace(/"/g,"&quot;")}" ${v} />
        ${r}
      </label>
    `}),c+="</div>",i.innerHTML=c;const p=i.querySelector(".multi-select-toggle"),l=i.querySelector(".multi-select-dropdown");p.addEventListener("click",r=>{r.stopPropagation(),l.classList.toggle("open")}),l.addEventListener("change",()=>{const r=Array.from(l.querySelectorAll("input:checked")).map(v=>v.value);s(r)})}async function m(){const a=document.getElementById("gallery");if(a.innerHTML="",!t.result||!t.result.entries||t.result.entries.length===0){a.innerHTML='<div class="banner info">No entries found.</div>';return}for(let e=0;e<t.result.entries.length;e++){const n=t.result.entries[e],d=document.createElement("div");d.className="entry-card",a.appendChild(d);const s=document.createElement("div");s.className="img-col",s.innerHTML='<div class="spinner"></div>',d.appendChild(s);const i=document.createElement("div");d.appendChild(i);const o=n.image_path.split(/[\\/]/).pop();let c=`<div class="filename-box">🖼️ ${o}</div>`;const p=[];n.created_at&&p.push(`Created: ${new Date(n.created_at).toLocaleString()}`),n.modified_at&&p.push(`Modified: ${new Date(n.modified_at).toLocaleString()}`),p.length>0&&(c+=`<div class="timestamp-box">${p.join(" · ")}</div>`),c+=`<div class="image-meta-box last" id="meta-${e}">Loading meta...</div>`,c+=`<div class="prompt-label">Prompt: ${n.prompt||"N/A"}</div>`,t.editingIndex===e?c+=`
